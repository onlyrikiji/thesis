...
# Read search queries from CSV and create a regex pattern
search_queries = []
with open('search_queries.csv', mode='r') as file:
    reader = csv.reader(file)
    for row in reader:
        if row:
            search_queries.append(re.escape(row[0]))  # Escape to safely use in regex

# Combine search queries into a single pattern
pattern = '|'.join(search_queries)  # This will create a pattern that matches any of the search queries
...
cursor.execute("SELECT id, content FROM post ORDER BY RANDOM() LIMIT 10")
